...
## Step 9: 正例 vs 負例の特徴量比較（2026-04-20実施）

### 負例の構築方法

issue_complexity=1 のコメントが追加されたファイル（同commit・同file）に存在した**他のメソッド**を負例とした（`build_negative_samples.py`）。

- 対象：423ユニークな (repo, commit, file) タプル
- GitHub API でcommit時点のファイル内容を取得 → Lizardで全メソッド抽出 → 正例と一致するものを除外
- 負例総数：**17,498件**

### 正例 vs 負例の特徴量比較

| 特徴量 | 正例（n=820） | 負例（n=17,498） | 比率（正/負） |
|---|---|---|---|
| cyclomatic_complexity | 1.46 | 2.07 | 0.71 |
| loc | 6.73 | 7.72 | 0.87 |
| parameter_count | 1.06 | 1.51 | 0.70 |
| avg_identifier_length | 9.13 | 7.79 | 1.17 |
| **abbrev_ratio** | **0.073** | **0.186** | **0.39** |
| is_public | 0.926 | 0.847 | 1.09 |
| **return_type_primitive** | **0.125** | **0.396** | **0.32** |
| **method_name_word_count** | **2.66** | **2.02** | **1.32** |
| throws_count | 0.038 | 0.019 | 1.95 |

### 主要な発見

1. **return_type_primitive（比率0.32）**：最も識別力が高い。コメントが必要なメソッドはドメイン型（`CacheStats`・`Range`・`Duration`）を返す。プリミティブ型（`int`/`boolean`）は意味が自明なのでコメントが追加されにくい。

2. **abbrev_ratio（比率0.39）**：正例は略語が少ない（丁寧な命名）。良く命名されていてもセマンティクスが不明確なAPIは説明が必要になる。

3. **method_name_word_count（比率1.32）**：正例はメソッド名の語数が多い（`getMaximumExpectedValue`=4語等）。長い名前は複雑な概念を表している。

4. **cyclomatic/LOC/parameter（0.7台）**：正例のほうが**シンプル**。構造的複雑さは負例（コメント不要）の方が高い。「複雑なコードにコメントが必要」という直感に**反する**。

### 結論

「コメントが必要なコード」は**構造的に複雑なコードではなく、セマンティクスが不明確な公開APIメソッド**。特に戻り値がドメイン型（非プリミティブ）のメソッドに集中している。

---

## 残タスク

### 正例再構築（2026-04-24 方針変更）

issue_complexity フィルタを廃止し，**auto-closeキーワード × issue実際にclosed** を正例の条件とする。

**根拠：**
- 「コメントのみコミットがissueをcloseした」という事実だけで「コメントが解決策だった」が成立する
- issueの内容（難解さかどうか）を人間が判定する必要がなくなる → 主観性の排除
- サンプル数が210件（issue_complexity=1）から大幅に増える見込み

**実装済み：** `filter_autoclose.py`
- auto-closeキーワード（fixes/closes/resolves）の検出
- GitHub APIでissueのstate=closedを確認
- DBの `is_autoclose`・`issue_closed` カラムに記録
...
python filter_autoclose.py
```

実行後，以下を確認して次の分析に進む：
- 正例候補数（is_autoclose=1 AND issue_closed=1）
- 十分な数が得られたら annotate.py → 特徴量比較 → 分類器構築

- [ ] `filter_autoclose.py` をMac Miniで実行して正例候補数を確認
- [ ] 新しい正例で特徴量（annotate.py）を再計算
- [ ] 新正例 vs 負例の特徴量比較（Step 9の再実施）
- [ ] 予測モデルの構築（現特徴量でのbaseline分類器）
- [ ] より多くのリポジトリ・言語への一般化検証
